...
import { STOCKIST_ADDRESS_ADD_API } from '../../../config/api.config';
import { StockistAddress } from '../../../entities/StockistAddressEntity';
import { StockistDropdown, ServiceableStateDropdown } from '../../general/GeneralComponents';

const StockistAddressAdd = (props: {hideTitle?: boolean, isDialog?: boolean}) => {

    const title = "Stockist Addresses";
    const navigate = useNavigate();

    const formik = useFormik<Partial<StockistAddress>>({
        initialValues: {
            addressTitle: ""
        },
        validate: (data) => {
        ...
        onSubmit: (data) => {
            (async () => {
                await postApiData<Partial<StockistAddress>>(STOCKIST_ADDRESS_ADD_API, data);
                if(!props.isDialog){
                    navigate(-1);
                }
            }
...
export default StockistAddressAdd;
